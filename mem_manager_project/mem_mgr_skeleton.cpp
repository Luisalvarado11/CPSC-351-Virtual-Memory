...
//@Breif: set tlb[index] to entry
void tlb_add(int index, page_node entry) {
    tlb[index] = entry;
 }  // DONE

//@Breif: Remove the pages and check if it is in tlb
void tlb_remove(int index) {
    tlb[index].npage = - 1;
    tlb[index].is_present = false;
 }  // DONE

//@Breif: checks to see if the page is in the frame and if it is then hit
void tlb_hit(size_t& frame, size_t& page, size_t& tlb_hits, int result) {
    //checks to see if page is in the tlb
    result = check_tlb(page);
    //if it is in the tlb then hit
    tlb_hits++;
    //gets the pages frame number from the tlb
    frame = tlb[result].frame_num;
    //sets page to the numer of page and holds that information
    page = tlb[result].npage;
    //sets page to true since it is being used.
    pg_table[page].is_used = true;
  }  // DONE

//@Brief: checks to see if the page is in the frame and if it is not then "miss"
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) {
    //frame is being set up with the frame number
    frame = pg_table[page].frame_num;
    //if the page is being used then return true
    pg_table[page].is_used = true;
    //adding the pages to the table
    tlb_add(tlb_track++, pg_table[page]);
    //ensures that it does not surpass the table size of 16
    tlb_track %= TLB_SIZE;
 } // DONE

//@Breif: First-In-First-Out. Remove the page that has been there the longest. 
void fifo_replace_page(size_t& frame ) {
    //find the page in the page table
    size_t page = find_frame_ptable(frame);
    //if page is not present then set to false
    pg_table[page].is_present = false;
    //check if page is being used
    pg_table[page].is_used = false;
 }   // TODO

//@Breif: Least Recent Used. Remove the page that hasn't been referenced in a while.
...
void check_address_value(size_t logic_add, size_t page, size_t offset, size_t physical_add,
    ...
    printf("log: %5lu 0x%04x (pg:%3lu, off:%3lu)-->phy: %5lu (frm: %3lu) (prv: %3lu)--> val: %4d == value: %4d -- %s", 
          logic_add, unsigned(logic_add), page, offset, physical_add, frame, prev_frame, 
